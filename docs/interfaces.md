...
Configure your module under `protocol_config.client_interface_module` in the scenario. Your module should be a `gen_server` with this contract (see `psmark_default_mqtt_interface` for a full reference implementation):
...
- Handle calls (forwarded by `psmark_erlang_mqtt_adapter`):
...
- Use `psmark_utils:generate_mqtt_payload_data/3` to produce `{SeqId, Payload}` which embeds sequence and publisher ID. The adapter extracts metrics via `decode_seq_header_with_publisher/1`.
...
The DDS adapter (`psmark_nif_dds_adapter`) calls a NIF module to interact with the DDS runtime. Provide a module that exports these functions (see `psmark/src/protocol_clients/default_interfaces/psmark_default_dds_interface.erl`):
...
{protocol_config,
 [
   ...
   {domain_id, 42},
   {qos_profile, ""},
   {config_file, "configs/dds_configs/psmark_default_dds_interface.ini"}
 ]}
```
## Advanced: Adding a New Protocol
If you want to add an entirely new protocol:
- Extend `?SUPPORTED_PROTOCOLS` in `psmark/include/psmark_config.hrl`.
- Teach `psmark_config_manager` how to parse your protocol’s `protocol_config` and provide accessor functions.
- Add a new adapter module analogous to `psmark_erlang_mqtt_adapter` or `psmark_nif_dds_adapter`.
- Update `psmark_client_sup` with a `process_<yourproto>_children/1` to supervise your client workers.
